...
2. 작성한 코드는 `--$ YYMMDDhhmm (변경 이유 1줄)` 주석을 영어로 작성. 한 작업(change set)에 속하는 모든 파일·모든 라인은 **동일한 타임스탬프 태그**를 사용 — 나중에 grep으로 찾기 쉽게. ex) `--$ 2605061430 Add ILA for offset path`
   - 접두사는 `--$` 통일 (`--#` 사용 금지)
   - 타임스탬프는 작업 시작 시점에 1번 정해서 그 작업 내 모든 파일/라인에 동일 태그로 기록
3. 신규 변수 선언 시 기존 변수 선언한 것 참고하여 통일성있게 작성
4. CDC 코드 작성 시 process 별도로 분리하여 작성
5. .vhd/.c/.h 코드만 수정하기 나머지 확장자는 수정하는 방법 가이드라인으로 제시
6. 파일 수정 전에는 반드시 워크스페이스 루트의 `backup/` 디렉토리에 `<원본파일명>_YYMMDDHHMM.<확장자>.bak` 형태(타임스탬프 10자리, 평탄 구조)로 백업 파일을 생성할 것 (예: `TOP_HEADER.vhd` → `backup/TOP_HEADER_2605061337.vhd.bak`)
7. FPGA(VHDL) 파일 수정 시 `TOP_HEADER.vhd` 의 변경 이력 주석에 날짜와 수정 내용을 기존 형식에 맞추어 추가할 것
8. VITIS FW 파일 수정 시 `vitis/EXTREAM_fw/src/main.c` 의 변경 이력 주석에 날짜와 수정 내용을 기존 형식에 맞추어 추가할 것
9. **모든 코드 주석(VHDL/C 공통)은 영어로 작성** — 변경 이력(`--$ YYMMDDhhmm ...`) + 인라인 코멘트 + TODO/FIXME 모두. 한글 주석 금지. (사용자에게 출력하는 메시지/UART 출력은 한국어 가능, 코드 내부 주석만 해당)
10. **변경 이력 vs 상세 설명 분리 원칙**:
    - `main.c` / `TOP_HEADER.vhd` 의 변경 이력 줄은 **30자 내외 한 줄 요약**으로, 반드시 **수정 대상 함수/모듈명을 포함**하여 grep으로 위치 추적이 쉬워야 함 (예: `//$ 2605181040 int_handler: FB overflow UART msg`)
    - 상세 설명(목적/배경/사용 레지스터/전제조건/반환값 등)은 **실제 수정·신규 추가된 코드 위치**에 블록 주석으로 작성하되 **3~5줄 이내로 간결하게** (장황한 다단 항목 금지)
    - 신규 함수도 함수 직전에 3~5줄 이내 요약 영문 블록 주석 (Purpose / Why / Reference 등 핵심만)
    - 모든 상세 주석 블록 첫 줄에도 동일 변경 태그(`//$ YYMMDDhhmm` 또는 `--$ YYMMDDhhmm`)를 붙여서 main.c 이력과 grep으로 연결 가능하게 할 것
11. **타임스탬프는 작업 시작 시 현재 시각으로 확정**:
    - 작업 시작 시점에 시스템 시각(예: `date "+%y%m%d%H%M"`)을 1번 조회해서 그 값을 그 change-set 전체(코드 태그 `//$ YYMMDDhhmm` / 백업 파일명 `*_YYMMDDHHMM.<확장자>.bak` / 이력 라인)에 동일하게 사용. 작업 진행 중 다른 시각으로 바꾸지 말 것
    - 이력 라인의 사람-읽기용 날짜(`YYYY.MM.DD hh:mm`) 도 동일 시각 사용
    - **이력 라인은 항상 마지막 줄에 직전 번호+1 형태로 append** (중간 삽입 금지)
    - **같은 change-set 시각으로 `FW_DATE` (`vitis/EXTREAM_fw/src/main.c` 상단 `u8 FW_DATE [20] = "YYYY.MM.DD hh:mm";`) 도 함께 갱신**. VHDL-only 변경이면 대신 `TOP_HEADER.vhd` 의 `FPGA_DATE` 를 동일 기준으로 갱신
12. **변경 결과를 사용자에게 보고할 때 줄번호 하이퍼링크 사용**:
    - 수정/추가된 코드 위치를 보여줄 때 항상 `[파일명:줄번호](상대경로#Lstart-Lend)` 또는 `[파일명:줄번호](상대경로#Lline)` 형식의 마크다운 링크로 표기. 단일 라인이면 `#L42`, 범위면 `#L42-L51`
    - 사용자가 IDE에서 클릭만으로 해당 위치로 이동할 수 있어야 함 — 단순히 파일명만 적거나 줄번호 없이 적으면 안 됨
    - 변경된 모든 주요 지점(블록 주석 헤더 / 신규 함수 본문 / 수정된 분기 / FW_DATE 갱신 / 이력 라인 등)을 각각 줄번호 링크로 명시