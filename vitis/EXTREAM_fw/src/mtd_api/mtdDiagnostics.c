...
/* defines for 1.0.5:2 for 33X0/E20X0 devices for speed selection */
...
)
{
    ...
    /* sets speed bits 1.0.5:2 for 33X0/E20X0 devices */
    switch (speed)
    {
    ...
    {
        /* speed is 10G or (5G/2.5G on X33X0/E20X0) */
...
)
{
    ...
    {
        return MTD_OK; /* don't touch it on E21X0, already enabled by firmware */
    }
    ...
    if (MTD_IS_X32X0_BASE(devPtr->deviceId) || MTD_IS_X33X0_BASE(devPtr->deviceId))
    {
        ...
        /* Enable Cunit temperature sensor */
        if (mtdHwSetPhyRegField(devPtr,sensorPort,MTD_CUNIT_TEMP_SENSOR_CTRL,14,2,0) != MTD_OK)
        {
...
)
{
    ...
    if (MTD_IS_E21X0_BASE(devPtr->deviceId))
    {
        ATTEMPT(mtdHwXmdioRead(devPtr,sensorPort,MTD_TUNIT_TEMP_SENSOR,&temp));
    }
    else if (MTD_IS_X32X0_BASE(devPtr->deviceId) || MTD_IS_X33X0_BASE(devPtr->deviceId))
    {
        ATTEMPT(mtdHwXmdioRead(devPtr,sensorPort,MTD_CUNIT_TEMP_SENSOR,&temp));
    }
    else
    {
