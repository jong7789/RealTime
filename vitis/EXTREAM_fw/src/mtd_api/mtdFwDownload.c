...
{    
    ...
    *******************************************************************************/     
    ATTEMPT(mtdMdioRamDownload(devPtr,slaveData,slaveSize,port,use_ram_checksum,MTD_FALSE,0,errCode));     

    /* make sure the slave code started */
    TRY_IO(mtdHwGetPhyRegField(devPtr,port,MTD_BOOT_STATUS_REG,4,1,&tmp));
    ...
    *******************************************************************************/
    ...
    TRY_IO(mtdHwXmdioWrite(devPtr,port,COMMAND_REG,VERIFY_FLASH));

    TRY_IO(mtdHwXmdioRead(devPtr,port,COMMAND_REG,&tmp));
    while(tmp == VERIFY_FLASH || tmp == MTD_SLAVE_FLASH_BUSY)
    {
        ATTEMPT(mtdWait(devPtr,500));  /* Don't poll phy too frequently, 
    ...
    OUT MTD_U16 *errCode)
{
    ...
    *******************************************************************************/
    ...
    use_ram_checksum = 1; // All X32X0 and X33X0 have a ram checksum register
    ...
    *******************************************************************************/
    ...
    ATTEMPT(mtdMdioRamDownload(devPtr,&appData[MTD_HEADER_SIZE],(appSize-MTD_HEADER_SIZE), \
                           port, use_ram_checksum,MTD_TRUE,~expectedChecksum,errCode));
    ...
    OUT MTD_U16 *errCode)
{
    ...
    *******************************************************************************/
    ...
    ATTEMPT(mtdParallelMdioRamDownload(devPtr,&appData[MTD_HEADER_SIZE],(appSize-MTD_HEADER_SIZE), ports, \
                                       numPorts, erroredPorts,use_ram_checksum,MTD_TRUE,~expectedChecksum,errCode));    
    ...
    for(i = 0; i < numPorts; i++)
    {
        if(erroredPorts[i] == 0)
        {
            TRY_IO(mtdHwGetPhyRegField(devPtr,ports[i],MTD_BOOT_STATUS_REG,4,1,&tmp));
...
)
{
    ...
    *******************************************************************************/
    ...
    ATTEMPT(mtdParallelMdioRamDownload(devPtr,slaveData,slaveSize, ports, \
                                       numPorts, erroredPorts, use_ram_checksum, MTD_FALSE, 0,errCode));   
    ...
    *******************************************************************************/
    ...
    TRY_IO(mtdHwXmdioWrite(devPtr,ports[0],COMMAND_REG,VERIFY_FLASH));
    ...
    for(i = 0; i < numPorts; i++)
    {
        if(erroredPorts[i] == 0)
        {
            ...
            while(tmp == VERIFY_FLASH || tmp == MTD_SLAVE_FLASH_BUSY)
            {
                ATTEMPT(mtdWait(devPtr,100));  /* Don't poll phy too frequently, 
