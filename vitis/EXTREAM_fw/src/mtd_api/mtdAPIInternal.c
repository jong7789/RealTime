/*******************************************************************************
Copyright (C) 2014 - 2017, Marvell International Ltd. and its affiliates
                                ...
                                MTD_U8 data[],MTD_U32 size, MTD_U16 *errCode)
{
    ...
    *errCode = 0;
...
#endif
    TRY_IO(mtdHwXmdioWrite(devPtr,port,COMMAND_REG, ERASE_FLASH_PROGRAM_AREA));

    TRY_IO(mtdHwXmdioRead(devPtr,port,COMMAND_REG,&tmp));
    ...
    {        
        ...
        TRY_IO(mtdHwXmdioWrite(devPtr,port,COMMAND_REG,FILL_BUFFER));
        ...
        while(byteIndex < stopIndex)
        {
            ...
            buf_checksum += value;
            TRY_IO(mtdHwXmdioWrite(devPtr,port,DATA_REG,value));
        }
        ...
        TRY_IO(mtdHwXmdioRead(devPtr,port,CHECKSUM_REG,&tmp_checksum));
        TRY_IO(mtdHwXmdioRead(devPtr,port,WORDS_RCVD_REG,&words_rcvd));
        ...
        TRY_IO(mtdHwXmdioWrite(devPtr,port,COMMAND_REG,WRITE_BUFFER));
    ...
    if (lastTransferSize)
    {
        ...
        while(tmp == WRITE_BUFFER || tmp == MTD_SLAVE_FLASH_BUSY)
        {
            mtdWait(devPtr,500);  /* this can take several 2-3 seconds,
...
MTD_STATUS mtdMdioRamDownload(MTD_DEV_PTR devPtr,MTD_U8 data[],
                              MTD_U32 size, MTD_U16 port, MTD_U8 use_ram_checksum, MTD_BOOL use_hdr_checksum,
                              ...
                              MTD_U16 *errCode) 
{
    ...
    *errCode = 0;
    ...
    TRY_IO(mtdHwXmdioRead(devPtr,port,MTD_BOOT_STATUS_REG,&tmp));
    ...
    if(use_ram_checksum)
    {
        ...
        TRY_IO(mtdHwXmdioRead(devPtr,port, MTD_RAM_CHECKSUM_REG,&ram_checksum));        
    }

    /* Set starting address in RAM to 0x00100000 */
    TRY_IO(mtdHwXmdioWrite(devPtr,port,MTD_LOW_WORD_REG,0));
    TRY_IO(mtdHwXmdioWrite(devPtr,port,MTD_HI__WORD_REG,0x0010));
    ...
    if(use_ram_checksum)
    {
        ...
        {
#if DB_TEST_FLASHCODE
            MTD_DBG_INFO( "Error downloading code. Expected RAM HW checksum to be 0x%04X but it was 0x%04X", (int)expected_checksum, (int)ram_checksum);
#endif
            *errCode = MTD_RAM_HW_CHECKSUM_ERR;
            return MTD_FAIL;
        }

        if (use_hdr_checksum && (ram_checksum != hdr_checksum))
        {
#if DB_TEST_FLASHCODE
            MTD_DBG_INFO( "Error downloading code. Expected RAM HDR checksum to be 0x%04X but it was 0x%04X", (int)hdr_checksum, (int)ram_checksum);
#endif
...
MTD_STATUS mtdParallelMdioRamDownload(MTD_DEV_PTR devPtr,MTD_U8 data[],MTD_U32 size,
                                      ...
                                      MTD_U8 use_ram_checksum, MTD_BOOL use_hdr_checksum, MTD_U16 hdr_checksum, MTD_U16 *errCode) 
{
    unsigned     buffCount;
    MTD_U8 lowByte, highByte;
    MTD_U16 tmp;
    ...
    *errCode = 0;
    ...
    for(i = 0; i < numPorts; i++)
    {
        ...
        ATTEMPT(mtdPutPhyInMdioDownloadMode(devPtr,ports[i])); /* if fails, returns errCode of 0 */
    }
    ...
    for(i = 0; i < numPorts; i++)
    {
        TRY_IO(mtdHwXmdioRead(devPtr,ports[i],MTD_BOOT_STATUS_REG,&tmp));
        if (tmp != 0x000A)
        {
#if DB_TEST_FLASHCODE
    ...
    TRY_IO(mtdHwXmdioWrite(devPtr,ports[0],MTD_LOW_WORD_REG,0));
    TRY_IO(mtdHwXmdioWrite(devPtr,ports[0],MTD_HI__WORD_REG,0x0010));
    ...
    if(use_ram_checksum) /* always true for X32X0 */
    {
        for(i = 0; i < numPorts; i++)
        {
            ...
            TRY_IO(mtdHwXmdioRead(devPtr,ports[i],MTD_RAM_CHECKSUM_REG,&ram_checksum[i])); 
                                 ...
                                 MTD_U16 *errCode)
{
    ...
    for(i = 0; i < numPorts; i++)
    {
        TRY_IO(mtdPutPhyInParallelWriteMode(devPtr,ports[i]));  /* parallel write */
    }

    TRY_IO(mtdHwXmdioWrite(devPtr,ports[0],COMMAND_REG, ERASE_FLASH_PROGRAM_AREA));

    TRY_IO(mtdRemovePhyFromParallelWriteMode(devPtr,ports[0]));   /*remove parallel write*/

    for(i = 0; i < numPorts; i++)
    {
        if(erroredPorts[i] == 0)
        {
            TRY_IO(mtdHwXmdioRead(devPtr,ports[i],COMMAND_REG,&tmp));
            while( tmp==ERASE_FLASH_PROGRAM_AREA || tmp==MTD_SLAVE_FLASH_BUSY )
            {
                ATTEMPT(mtdWait(devPtr,100)); /* don't poll too often, this can take awhile */
                TRY_IO(mtdHwXmdioRead(devPtr,ports[i],COMMAND_REG,&tmp));
            }
    ...
    for(transferIndex=0; transferIndex < numTransfers; transferIndex++)
    {
        ...
        /* Tell the slave we've written the start address and size
        / and now we're going to start writing data*/
        TRY_IO(mtdHwXmdioWrite(devPtr,ports[0],COMMAND_REG,FILL_BUFFER));

        mtdRemovePhyFromParallelWriteMode(devPtr,ports[0]);   
        ...
        while(byteIndex < stopIndex)
        {
            ...
            value |= (((MTD_U16)data[byteIndex++]) << 8);
            buf_checksum += value;
            TRY_IO(mtdHwXmdioWrite(devPtr,ports[0],DATA_REG,value));
        }
        ...
        for(i = 0; i < numPorts; i++)
        {
            ...
            {
                /* check and see if we can go on to the write*/
                TRY_IO(mtdHwXmdioRead(devPtr,ports[i],CHECKSUM_REG,&tmp_checksum));
                TRY_IO(mtdHwXmdioRead(devPtr,ports[i],WORDS_RCVD_REG,&words_rcvd));
        ...
        for(i = 0; i < numPorts; i++)
        {
            if (erroredPorts[i] == 0)
            {
                TRY_IO(mtdHwXmdioWrite(devPtr,ports[i],COMMAND_REG,WRITE_BUFFER));
            }
        }
        ...
        for(i = 0; i < numPorts; i++)
        {
            if(erroredPorts[i] == 0)
            {
                ...
                else
                {
                    ...
                    {
                        /* readback checksum of what was stored in flash */
                        TRY_IO(mtdHwXmdioRead(devPtr,ports[i],CHECKSUM_REG,&reported_checksum)); 
                ...
                TRY_IO(mtdHwXmdioRead(devPtr,ports[i],WORDS_WRITTEN_REG,&tmp));
    ...
    if (lastTransferSize)
    {
        ...
        {        
            /* Set the flash start address */
            TRY_IO(mtdHwXmdioWrite(devPtr,ports[i],LOW_ADDRESS_REG, (MTD_U16)byteIndex));
            TRY_IO(mtdHwXmdioWrite(devPtr,ports[i],HIGH_ADDRESS_REG, (MTD_U16)(byteIndex>>16)));
            /* Set the size of the buffer we're going to send */
            TRY_IO(mtdHwXmdioWrite(devPtr,ports[i],ACTUAL_BUFF_SIZE_IN_REG, (MTD_U16)(lastTransferSize/2)));
            /* Tell the slave we've written the start address and size */
            /* and now we're going to start writing data */
            TRY_IO(mtdHwXmdioWrite(devPtr,ports[i],COMMAND_REG,FILL_BUFFER));
        }
        ...
        MTD_DBG_INFO("\nTelling slave to save %d bytes of last buffer\n", (int)lastTransferSize); 
#endif
        TRY_IO(mtdHwXmdioWrite(devPtr,ports[0],COMMAND_REG,WRITE_BUFFER));
