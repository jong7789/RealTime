...
/******************************************************************************
 ...
 Outputs:
    ...
    information about the error. If it does, it will be one of the following:
         ...
         App checksum did not match checksum in header
...
Example:
    ...
    fileSize = mtdOpenReadFile(devPtr,port, &hFile, "image_name.hdr", appData,
                                    (MTD_MAX_APP_SIZE+MTD_HEADER_SIZE));
    retStatus = mtdParallelUpdateRamImage(devPtr,port, 
...
/******************************************************************************       
MTD_U16 mtdParallelUpdateFlashImage(MTD_DEV_PTR devPtr,
 ...
 Example:
    ...
    slaveFileSize = mtdOpenReadFile(devPtr,port, &sFile, 
                                    ...
                                    MTD_MAX_SLAVE_SIZE);
    retStatus = mtdParallelUpdateFlashImage(devPtr,port, appData, 
                                            fileSize, slaveCode, 
