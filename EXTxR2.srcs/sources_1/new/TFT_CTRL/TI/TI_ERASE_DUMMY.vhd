...
architecture Behavioral of TI_ERASE_DUMMY is
...
begin
                ...
                case (state_dpram) is
                    ...
                    when s_WAIT =>
                        if (swr_end_trig = '1') then
                            ...
                            senb    <= '1';
                            saddrb0 <= conv_std_logic_vector(3, 10);  -- 10/4 + 1
                            saddrb1 <= conv_std_logic_vector(3, 10);  -- 10/4 + 1
                            saddrb2 <= conv_std_logic_vector(2, 10);  -- 10/4
                            saddrb3 <= conv_std_logic_vector(2, 10);  -- 10/4
                    ...
                    when s_DATA =>
                        if (shcnt = sreg_width - 1) then
                            ...
                            senb    <= '0';
                            saddrb0 <= conv_std_logic_vector(3, 10);
                            saddrb1 <= conv_std_logic_vector(3, 10);
                            saddrb2 <= conv_std_logic_vector(2, 10);
                            saddrb3 <= conv_std_logic_vector(2, 10);
                            shsync  <= '0';
                            shcnt   <= (others => '0');
                        else
                            shcnt <= shcnt + '1';
    ...
    --### select up/down line
    --# address reverse for upper/lower line process
    process (iui_clk)
    begin
        if (iui_clk'event and iui_clk = '1') then
--            if MODE_LOWER_NORMAL = '1' then
                if ROIC_DUAL_BY_MODEL(GNR_MODEL) = 1 then
                    saddrb0_ud <= saddrb0;
                    saddrb1_ud <= saddrb1;
                    saddrb2_ud <= saddrb2;
                    saddrb3_ud <= saddrb3;
