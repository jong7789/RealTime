...
begin
    ...
    --# Latch 3x3 neighbor data and defect mask into arrays
    process(idata_clk)
    begin
    ...
    --# Compute defect-corrected output with divisor lookup
    process(idata_clk)
    begin
        if (idata_clk'event and idata_clk = '1') then
            if (idata_rstn = '0') then
    ...
    --# Delay input sync/count/data by 1 clock for pipeline alignment
    process(idata_clk)
    begin
        if (idata_clk'event and idata_clk = '1') then
            if (idata_rstn = '0') then
